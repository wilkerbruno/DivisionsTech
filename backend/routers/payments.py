...
def create_checkout(req: CheckoutRequest, db=Depends(get_db)):
    ...
    # Registra cliente como pendente
    result = db.execute(
        text("""
            INSERT INTO clients
                (name, cpf, email, phone, plan, domain, has_domain, status, payment_status)
            VALUES
                (:name, :cpf, :email, :phone, :plan, :domain, :has_domain, 'pending', 'pending')
        """),
        {
            "name":       req.name,
            "cpf":        req.cpf or None,
    ...
    preference_data = {
        "items": [{
            ...
            "description": f"Plano {plan_info['name']} de hospedagem mensal",
            "quantity":    1,
    ...
    mp_result = sdk.preference().create(preference_data)
    if mp_result["status"] != 201:
        raise HTTPException(status_code=500, detail="Erro ao criar preferência no Mercado Pago")

    pref = mp_result["response"]
