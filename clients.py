...
def list_clients(
    ...
    if search:
        q += """ AND (
            name  LIKE :s OR email LIKE :s OR
            phone LIKE :s OR cpf   LIKE :s
        )"""
        params["s"] = f"%{search}%"
    if status:
        q += " AND status = :status"
...
def update_client(client_id: int, data: ClientUpdate,
                  db=Depends(get_db), _: str = Depends(get_current_admin)):
    ...
    fields["id"] = client_id
    db.execute(text(f"UPDATE clients SET {set_clause} WHERE id = :id"), fields)
    db.commit()
    return {"ok": True}
